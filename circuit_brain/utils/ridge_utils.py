...
def ridge(x, y, lam):
    nfeats = x.shape[1]
    return torch.matmul(
        tla.inv(torch.matmul(x.T, x) + lam * torch.eye(nfeats)), torch.matmul(x.T, y)
    )


def ridge_lam_per_target(x, y, x_val, y_val, lams=[1e-4, 1e-3]):
    error = torch.zeros(len(lams), y.shape[1])
...
def cross_val_ridge(x_train, y_train, n_splits=10, lams=[1e-3, 1e-4]):
    r_cv = torch.zeros(len(lams), y_train.shape[1])
    ...
    weights = torch.zeros((x_train.shape[1], y_train.shape[1]))

    for i in range(len(lams)):
        y_label_idx = min_lams == i
        weights[:, y_label_idx] = ridge(x_train, y_train[:, y_label_idx], lams[i])
